...
const sliderBtnStart = document.querySelector('.our-friends__slider-btn--start');
const sliderBtnEnd = document.querySelector('.our-friends__slider-btn--end');
const sliderBtnPageNum = document.querySelector('.our-friends__slider-btn--active');
let slideBtnNum = 1;
...
let countCardsOnSlide = window.innerWidth >= 1280 ? 8 : window.innerWidth >= 768 ? 6 : 3;
let countSlides = window.innerWidth >= 1280 ? 6 : window.innerWidth >= 768 ? 8 : 16;
for (let i = 0; i < countSlides; i++) {
  while (randomNumbers.length !== countCardsOnSlide) {
...
function createSlides() {
  for (let i = 0; i < countSlides; i++) {
    ...
    slide.className = 'our-friends__slide';
    slider.append(slide);
    for (let key of randomArr[i]) {
      let newCard = `<div class="our-friends__card" data-pet="${petsJson[key].name}">
      <img class="our-friends__card-img" src="img/pets-${(petsJson[key].name).toLowerCase()}.jpg" alt="Pet">
      <h4 class="our-friends__card-title">${petsJson[key].name}</h4>
      <a class="our-friends__card-link btn" href="##">Learn more</a>
      </div>`;
      slide.innerHTML += newCard;
    }
  }
  let diseaseLeft = 0;
  const slides = document.querySelectorAll('.our-friends__slide');
  for (let i = 0; i < slides.length; i++) {
    slides[i].style.left = `${diseaseLeft}%`;
    diseaseLeft += 100;
  }
  ...
  checkInactiveButtons();
}
createSlides();

let diseaseTransform = 0;

function showNextSlide() {
  if (slideBtnNum !== countSlides) {
    slideBtnNum++;
    sliderBtnPageNum.textContent = slideBtnNum;
    checkInactiveButtons();
    const slides = document.querySelectorAll('.our-friends__slide');
    diseaseTransform -= 100;
    slides.forEach(slide => slide.style.transform = `translateX(${diseaseTransform}%)`);
...
function checkInactiveButtons() {
  if (sliderBtnPageNum.textContent !== 1 || sliderBtnPageNum.textContent !== countSlides) {
    const inactiveButtons = document.querySelectorAll('.our-friends__slider-btn--inactive');
    inactiveButtons.forEach(el => el.classList.remove('our-friends__slider-btn--inactive'));
  }
  if (sliderBtnPageNum.textContent == 1) {
    sliderBtnStart.classList.add('our-friends__slider-btn--inactive');
    sliderBtnPrev.classList.add('our-friends__slider-btn--inactive');
  }
  if (sliderBtnPageNum.textContent == countSlides) {
    sliderBtnEnd.classList.add('our-friends__slider-btn--inactive');
    sliderBtnNext.classList.add('our-friends__slider-btn--inactive');
  }
}
...
function showLastSlide() {
  diseaseTransform -= (countSlides - sliderBtnPageNum.textContent) * 100;
  slideBtnNum = countSlides;
  sliderBtnPageNum.textContent = slideBtnNum;
  const slides = document.querySelectorAll('.our-friends__slide');
  slides.forEach(slide => slide.style.transform = `translateX(${diseaseTransform}%)`);
  checkInactiveButtons();
...
sliderBtnNext.addEventListener('click', showNextSlide);
sliderBtnPrev.addEventListener('click', showPrevSlide);
sliderBtnStart.addEventListener('click', showFirstSlide);
sliderBtnEnd.addEventListener('click', showLastSlide);
